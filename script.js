...
// Écouteur ultra-précis de Firebase
db.ref('.info/connected').on('value', function(snapshot) {
    // On combine le vrai réseau + la connexion Firebase
...
function syncOfflineData() {
    ...
    // On utilise Promise.all pour savoir quand TOUT est fini
    const syncPromises = queue.map(item => {
        return db.ref(item.path).set(item.data)
            .then(() => {
                console.log("Synchronisé avec succès:", item.path);
                return item.path; // On retourne le chemin réussi
            })
            .catch(err => {
                console.error("Échec de sync pour:", item.path, err);
                return null; // Échec
            });
    });
    ...
    Promise.all(syncPromises).then(results => {
        ...
        if (successfulPaths.length > 0) {
            // ON NETTOIE : On ne garde que ce qui n'a PAS été synchronisé
            ...
            alert(`✅ ${successfulPaths.length} mises à jour envoyées au Cloud. Mémoire locale libérée.`);
            
            // Forcer la mise à jour visuelle (les logos orange redeviendront verts/gris)
...
document.querySelectorAll(".room").forEach(room => {
    room.addEventListener("click", function() {
        ...
        showEquipments(this.dataset.floor, this.dataset.room);
    });
});
...
// ===== SAUVEGARDE RÉELLE (SANS CONFLIT DE MÉMOIRE) =====
let saveTimeout;
function saveReport(event, floor, room, eqIndex) {
    ...
    // ... (ton code de récupération des données reste le même) ...
    for (const [propName, propValue] of Object.entries(eq.details)) {
        const firebaseSafeKey = sanitizeKey(propName);
        const safeProp = sanitizeId(propName);
        let etat = "";
    ...
    if (isAppOnline) {
        // TENTATIVE DIRECTE SUR FIREBASE
        db.ref(firebasePath).set(finalSave).then(() => {
            // SUCCÈS : On s'assure que c'est effacé du local au cas où c'y était
            let queue = JSON.parse(localStorage.getItem('offlineQueue')) || [];
            ...
            // REPLI HORS-LIGNE EN CAS D'ERREUR SOUDAINE
...
function saveToOffline(path, data, btn) {
    let queue = JSON.parse(localStorage.getItem('offlineQueue')) || [];
    queue = queue.filter(q => q.path !== path); // Évite les doublons dans la queue
...
// ===== EXPORT PDF (FONCTIONNE MÊME HORS-LIGNE) =====
function exportToPDF() {
    const btn = document.querySelector('button[onclick="exportToPDF()"]');
    if(btn) btn.innerHTML = "⏳ Préparation...";

    // 1. On charge la copie locale de Firebase
    let allData = JSON.parse(localStorage.getItem('all_inspections')) || {};
    
    // 2. On charge les éléments modifiés hors-ligne qui ne sont pas encore sur Firebase
    const offlineQueue = JSON.parse(localStorage.getItem('offlineQueue')) || [];
    
    // 3. On fusionne les deux (la file d'attente hors-ligne écrase les anciennes données)
    offlineQueue.forEach(item => {
        const parts = item.path.split('/'); // ["inspections", "niveau1", "001", "Pompe_1"]
        const lvl = parts[1];
        const rm = parts[2];
